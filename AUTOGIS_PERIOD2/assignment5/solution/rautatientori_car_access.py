...
#data= data.loc[data.loc[:, 'walk_t']!=-1]
#data.columns
...
for i in range(len(names)):
    #data.loc[data['walk_t_ud'] == i, 'label_wt'] = names[i]
...
df = data[['x', 'y', 'car_r_t','car_r_t_ud',"from_id" ,'label_car']]

dfsource = ColumnDataSource(data=df)

# Include only coordinates from roads (exclude 'geometry' column)
rdf = roads[['x', 'y']]
#for a yet to be known reason, this started causing an error which was not the case before.
...
p = figure(title="Travel times to Helsinki city center by private car", tools=TOOLS,
...
grid = p.patches('x', 'y', source=dfsource, name="grid",
         ...
         fill_alpha=1.0, line_color="black", line_width=0.03, legend="label_car")



#the above did not work inititally until i installed bokeh using conda
#for i in names:
#    #paletet4=palette4[i]
#    #p.patches(legend=i, color= color_mapper)
#    p.patches(legend=i, color= palette4[names.index(i)])
#p.patches(legend='0-5', color="#084594")
#p.patches(legend='5-10', color="#2171b5")

# Add roads
r = p.multi_line('x', 'y', source=rdfsource, color="black")

# Add metro
m = p.multi_line('x', 'y', source=mdfsource, color="yellow")
...
output_file = r"C:\Users\oyeda\Desktop\AUTOGIS\AUTOGIS_PERIOD2\assignment5\solution\Rautatientori_car_access.html"
