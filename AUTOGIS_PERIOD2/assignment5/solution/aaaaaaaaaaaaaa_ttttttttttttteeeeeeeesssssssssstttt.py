# -*- coding: utf-8 -*-
"""
Created on Sun Dec 10 08:01:21 2017
...
df = data[['x', 'y', 'walk_t','walk_t_ud', 'from_id', 'label_wt']]
...
dfsource = GeoJSONDataSource(geojson=df.to_json())
...
grid = p.patches('x', 'y', source=dfsource, name="grid",
         ...
         fill_alpha=1.0, line_color="black", line_width=0.03, legend="label_wt")


# Add the lines to the map from our GeoJSONDataSource -object (it is important to specify the columns as 'xs' and 'ys')
r=p.multi_line('xs', 'ys', source=roads_source, color='gray', line_width=3)

# Add the lines to the map from our 'msource' ColumnDataSource -object
m=p.circle('x', 'y', source=point_source, color='black', size=6)
...
output_file = r"C:\Users\oyeda\Desktop\AUTOGIS\AUTOGIS_PERIOD2\assignment5\solution\test_Rautatientori_walk_access.html"
